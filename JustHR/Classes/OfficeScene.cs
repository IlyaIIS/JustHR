        ...
        public void GenerateNewCharacter()
        {
            ...
            {new List<string>
                ...
                },new List<string>
                {
                    ...
                    "Кроме того, я верю, что лучше всего человек работает на своем месте, а я идеально подхожу для этой должности, потому что так давно занимаюсь продажей продуктов... , знаю о них всё, и они мне даже во сне снятся"
                },new List<string>
                {
...
"Иванов Семён Артёмович",
"Чернышева Наталья Святославовна",
"Мухин Артём Романович",
"Попов Никита Сергеевич",
"Беляев Пётр Робертович",
...
"Макеев Артём Вадимович",
"Кузнецова Арина Егоровна",
"Ларина Мария Олеговна",
"Глухова Вера Артёмовна",
"Боброва Виктория Марковна",
"Токарев Николай Семёнович",
"Симонова Нина Фёдоровна",
...
"Сергеева Мария Фёдоровна",
"Яковлев Александр Артёмович",
"Иванов Павел Тимофеевич",
"Лапшин Даниил Александрович",
"Дьякова Анна Никитична",
"Никитин Юрий Артёмович",
...
"Прозрачный фон - вы ещё не интервьюировали этого человека. Если зеленый, вы приняли этого человека, если желтый то он вернется позже для повторного интервью. Красный, вы отказали ему.",
                ...
                else if (Objects[i].GetType() == typeof(Character))
                {
                    ...
                    if (Hour >= 18) //конец дня
                    {
                        if (Settings.Mentality < 25)
                            speech = new List<string> { "Всё, день закончился.", "Выглядишь неважно. Иди отдохни." };
                        ...
                        if (Settings.BossSatisfaction <= 0)
                            speech = new List<string> { "Ты мне не нравишься. теперь ты уволен.", "Да-да, уволен", "Я всё сказал" };
                        else if (dif > 1)
                    ...
                    if (Settings.Professionality <= 0)
                    {
                        if (!WasSecondChanceUsed)
                        {
                            ...
                            speech = new List<string> { "В компании всё плохо, все раздалбаи.", "Но я попросил их работать более усердно.", "Постарайся нанимать более квалифицированных сотрудников." };
                        } else
                        {
                    ...
                    } else if (Settings.Unity <= 0)
                    {
                        if (!WasSecondChanceUsed)
                        {
                            ...
                            WasSecondChanceUsed = true;
                            speech = new List<string> { "В компании всё плохо, все без конца ругаются.", "На плаву сможем оставаться только за счёт переработок.", "Постарайся нанимать более общительных сотрудников." };
                        }
                        else
                        {
